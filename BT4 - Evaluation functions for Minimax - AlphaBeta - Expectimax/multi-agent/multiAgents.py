...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        action = minimax_alpha_beta(gameState)
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def expectimax (state):
            # print("Number of Agents", state.getNumAgents())
            bestValue, bestAction = None, None
            print(state.getLegalActions(0))
            value = []
            for action in state.getLegalActions(0):
                #value = max(value,minValue(state.generateSuccessor(0, action), 1, 1))
                succ = exp_value(state.generateSuccessor(0, action), 1, 1)
                value.append(succ)
                if bestValue is None:
                    bestValue = succ
                    bestAction = action
        ...
        def exp_value(state, agentIdx, depth):
            if agentIdx == state.getNumAgents():
                return maxValue(state, 0, depth + 1)
            # print(agentIdx)
            value = 0.0
            ...
            for action in actions:
                value += p * exp_value(state.generateSuccessor(agentIdx, action), agentIdx + 1, depth)
            return value
        ...
        def maxValue(state, agentIdx, depth):
            ...
            value = None
            for action in state.getLegalActions(agentIdx):
                succ = exp_value(state.generateSuccessor(agentIdx, action), agentIdx + 1, depth)
                if value is None:
                    value = succ
                else:
                    value = max(value, succ)
...
def newBetterEvaluationFunction(currentGameState):
    ...
    return -2 * closestFood + ghost_distance - 10 * len(foodList) + closest_capsule + 3 * currentGameState.getScore()

# Abbreviation
better = newBetterEvaluationFunction
